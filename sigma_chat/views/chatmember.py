...
from sigma_core.serializers.chat_member import ChatMemberSerializer
...
class ChatMemberFilterBackend(BaseFilterBackend):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if not request.user.is_sigma_admin():
            ...
            # I can see a ChatMember if one of the following conditions is met:
            ...
            queryset = queryset.prefetch_related(
                Prefetch('user__memberships', queryset=ChatMember.objects.filter(is_accepted=True)),
...
class ChatMemberViewSet(viewsets.ModelViewSet):
    queryset = ChatMember.objects.select_related('group', 'user')
    ...
    def destroy(self, request, pk=None):
        ...
        try:
            modified_mship = ChatMember.objects.all().select_related('group').get(pk=pk)
            group = modified_mship.group
            my_mship = ChatMember.objects.all().get(group=group, user=request.user)
    ...
    def give_rights(self, request, pk=None):
        ...
        response_serializer: ChatMemberSerializer
        parameters_strategy:
            form: replace
        parameters:
        ...
        try:
            modified_mship = ChatMember.objects.all().select_related('group').get(pk=pk)
            my_mship = ChatMember.objects.all().get(group=modified_mship.group, user=request.user)
        except ChatMember.DoesNotExist:
            raise Http404()
        ...
        return Response(ChatMemberSerializer(modified_mship).data, status=status.HTTP_200_OK)
    ...
    def accept_join_request(self, request, pk=None):
        ...
        """
        try:
            gm = ChatMember.objects.select_related('group').get(pk=pk)
        except ChatMember.DoesNotExist:
            raise Http404()
